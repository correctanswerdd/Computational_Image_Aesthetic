...
class AVAImages:
    def __init__(self, type: str):
        """
        init
        :param type: 'tag' or 'score' or 'category'
    ...
    def load_data(self, filedir="AVA_dataset/AVA.txt", train_prob=0.96, test_prob=0.0001, val_prob=0.0001):
        ...
        :var self.train_set_x = 0
        :var self.train_set_y = 0
        :var self.test_set_x = 0
        :var self.test_set_y = 0
        :var self.val_set_x = 0
        :var self.val_set_y = 0
        """
        ...
        if self.type == 'score':
            ...
            print("loading validation images ... {st}->{ed}".format(st=int(total * (train_prob + test_prob)),
                                                                    ed=int(total * (train_prob + test_prob + val_prob))))
            self.val_set_x, self.val_set_y = self.urls_to_images(
                self.image_url[int(total * (train_prob + test_prob)): int(total * (train_prob + test_prob + val_prob))],
                self.cat[int(total * (train_prob + test_prob)): int(total * (train_prob + test_prob + val_prob))]
            )
