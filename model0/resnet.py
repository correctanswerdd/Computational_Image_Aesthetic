...
some functions:
* bottleneck: 基本残差单元
* resnet_v2: 相当于resnet网络构造器
* resnet_v2_block: block构造函数
...
def bottleneck(inputs, depth, depth_bottleneck, stride, rate=1,
    ...
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        if depth == depth_in:
            shortcut = resnet_utils.subsample(inputs, stride, scope='shortcut')
        else:
...
def resnet_v2_baseline(inputs,
                       ...
                       spatial_squeeze=True,
                       reuse=None,
                       scope='resnet_v2_baseline'):
    blocks = [
        resnet_v2_block('block1', base_depth=4, num_units=1, stride=2),
    ]
